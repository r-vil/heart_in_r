...
{
  return(paste("c'Ë/ ci sono", sum(is.na(dataset)), "valore/i NA"))
}
...
# ho rinominate secondo me nel modo pi˘ opportunoÏ
dataset <- dataset %>% 
  select(-one_of("x")) %>%
  rename(
    chest_pain = cp,
...
# - il tipo di relazione tramite re la bont‡ del modello tramite R^2; 
